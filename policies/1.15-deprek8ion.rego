...
_warn = msg {
  apis := ["apps/v1beta1", "apps/v1beta2"]
  input.spec.scaleTargetRef.apiVersion == apis[_]
  input.kind == "HorizontalPodAutoscaler"
  msg := sprintf("%s/%s: API %s has been deprecated, modify HorizontalPodAutoscaler input.spec.scaleTargetRef.apiVersion to use apps/v1 instead.", [input.spec.scaleTargetRef.kind, input.metadata.name, input.spec.scaleTargetRef.apiVersion])
}